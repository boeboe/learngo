...
func TestRun(t *testing.T) {
	testCases := []struct {
		...
		{name: "FilterExtMatch", root: "testdata",
			cfg:      config{ext: ".log", size: 0, list: true},
			expected: "testdata/dir.log\n"},
		{name: "filterExtNoMatch", root: "testdata",
			cfg:      config{ext: ".gz", size: 0, list: true},
			expected: ""},
		{name: "FilterExtSizeMatch", root: "testdata",
			cfg:      config{ext: ".log", size: 10, list: true},
			expected: "testdata/dir.log\n"},
		{name: "FilterExtSizeNoMatch", root: "testdata",
...
func createTempDir(t *testing.T, files map[string]int) (dirname string, clean func()) {
	t.Helper()
...
func TestRunDelExt(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := buffer.String()
			if tc.expected != res {
				t.Errorf("expected %q, but go %q instead\n", tc.expected, res)
			}
			...
			if len(lines) != expLogLines {
				t.Errorf("expcted %d log lines, got %d instead", expLogLines, len(lines))
			}
		})
	}
}
...
func TestRunArchive(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if expOut != res {
				t.Errorf("expected %q, got %q instad\n", expOut, res)
			}
